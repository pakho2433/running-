...
const email = normaliseEmail(process.argv[2] || process.env.READING_RUN_TEACHER_EMAIL || "");
const allowedDomains = String(process.env.READING_RUN_ALLOWED_TEACHER_DOMAINS || "")
  .split(",")
  .map((item) => item.trim().toLowerCase())
  .filter(Boolean);
const password = String(process.env.READING_RUN_TEACHER_PASSWORD || "").trim();
const displayName = String(process.argv[3] || process.env.READING_RUN_TEACHER_NAME || "Reading Run Teacher").trim();
const active = strictBoolean(process.env.READING_RUN_TEACHER_ACTIVE, "READING_RUN_TEACHER_ACTIVE", true);
...
if (!allowedDomains.some((domain) => email.endsWith(`@${domain}`))) throw new Error("Teacher email must use an approved school/TWGH domain");
if (password.length < 14) throw new Error("READING_RUN_TEACHER_PASSWORD must be at least 14 characters");
