...
setPersistence(auth, browserSessionPersistence).catch((error) => console.warn("Teacher persistence setup failed", error));
...
async function openCenter() { ui.modal.classList.remove("is-hidden"); document.body.classList.add("teacher-center-open"); await restoreTeacher(); if (state.authorized) { showDashboard(); await loadData(); } else showLogin(); }
...
async function loadData() { if (!state.authorized || state.loading) return; state.loading = true; ui.status.textContent = "正在載入已授權資料……"; try { const [students, logs, recommendations] = await Promise.all([getDocs(collection(db, "students")), getDocs(collection(db, "bookLogs")), getDocs(collection(db, "dailyRecommendations"))]); state.students = students.docs.map((item) => ({ id: item.id, ...item.data() })); state.logs = logs.docs.map((item) => ({ id: item.id, ...item.data() })); state.recommendations = recommendations.docs.map((item) => ({ dateKey: item.id, ...item.data() })); render(); renderRecommendations(); ui.status.textContent = `已安全載入 ${state.logs.length} 項閱讀紀錄。`; } catch (error) { console.error(error); ui.status.textContent = "資料載入失敗；請確認教師角色及 Firestore Rules 已部署。"; } finally { state.loading = false; } }
