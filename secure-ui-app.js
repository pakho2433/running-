...
const dom = {
  ...
  syncStatus: $("#syncStatus"), currentStudentLabel: $("#currentStudentLabel"), currentClassLabel: $("#currentClassLabel"),
  bookForm: $("#bookForm"), readingDate: $("#readingDate"), bookTitle: $("#bookTitle"), bookAuthor: $("#bookAuthor"),
  readingType: $("#readingType"), bookSubject: $("#bookSubject"), readingCompleted: $("#readingCompleted"),
  scorePreview: $("#scorePreview"), bookSubmit: $(".submit-reading-button"), myBooksCount: $("#myBooksCount"),
  myDistance: $("#myDistance"), myLastBook: $("#myLastBook"), classroomGrid: $("#classroomGrid"),
  trackTitle: $("#trackTitle"), trackRunnerCount: $("#trackRunnerCount"), trackLeader: $("#trackLeader"),
  locationButtons: $("#locationButtons"), currentLocationLabel: $("#currentLocationLabel"), trackRange: $("#trackRange"),
  trackCanvas: $("#trackCanvas"), trackEmpty: $("#trackEmpty"), leaderboardClass: $("#leaderboardClass"),
  leaderboardList: $("#leaderboardList"), runnerList: $("#runnerList"), toastRegion: $("#toastRegion"),
};
...
async function handleLogin(event) {
  ...
  if (!classId || !studentId || pin.length < 6) return loginMessage("請輸入學生 ID 及至少 6 位個人 PIN。", true);
  loginBusy(true);
  loginMessage("正在核對學生身份……");
  try {
    const user = await loginStudent(classId, studentId, pin);
...
function addPinField() {
  let input = $("#studentPin");
  if (!input) {
    const label = document.createElement("label");
    label.innerHTML = '<span>個人 PIN</span><input id="studentPin" type="password" inputmode="numeric" minlength="6" maxlength="30" autocomplete="off" placeholder="至少 6 位" required />';
    dom.studentId.closest("label")?.insertAdjacentElement("afterend", label);
    input = $("#studentPin");
  }
  const intro = $(".login-intro");
  if (intro) intro.textContent = "請使用學校派發的學生 ID 及個人 PIN 登入。";
  dom.loginButton.textContent = "安全登入";
  return input;
}
...
function readRecord() {
  return {
    ...
    title: clean(dom.bookTitle.value, 80), author: clean(dom.bookAuthor.value, 80),
    readingType: dom.readingType.value || "", subject: dom.bookSubject.value || "",
...
function clearForm() {
  dom.readingDate.value = schoolDateKey();
  dom.bookTitle.value = ""; dom.bookAuthor.value = "";
  dom.readingType.value = ""; dom.bookSubject.value = ""; dom.readingCompleted.value = "";
  updateScore();
}

function updateScore() { dom.scorePreview.textContent = String(scoreReading(readRecord())); }
function showLogin() { dom.appShell.classList.add("is-hidden"); dom.loginScreen.classList.remove("is-hidden"); sync("idle", "● 等待安全登入"); }
function loginBusy(value) { [dom.loginClass, dom.studentId, pinInput, dom.loginButton].forEach((item) => { if (item) item.disabled = value; }); }
...
function loginError(error) { if (error?.message === "PROFILE_MISMATCH") return "帳戶與班別或學生 ID 不符。"; if (["auth/invalid-credential", "auth/wrong-password", "auth/user-not-found"].includes(error?.code)) return "學生 ID 或 PIN 不正確。"; return "暫時未能登入，請檢查網絡。"; }
