# Reading Run 安全部署清單（400–500 人學校）

這個分支已把學生及教師資料權限改為角色制。合併程式碼並不等於安全設定已生效；必須完成以下 Firebase Console 及帳戶建立步驟。

## 1. 先在測試 Firebase 專案驗證

不要直接在正式學生資料庫測試。先建立 staging 專案，完成學生登入、教師登入、提交、每日上限、閱讀歷史及排行榜測試。

## 2. Authentication 設定

1. Firebase Console → Authentication → Sign-in method。
2. 啟用 **Email/Password**。
3. 建立學生帳戶及教師帳戶後，停用 **Anonymous** 登入。
...
`users` 文件只可由受信任的管理環境建立，不可由瀏覽器自行建立。
...
- 學生不能直接把里數改成任意數字。
- 每日第 6 次提交必須失敗。
- 非教師不能新增、修改或刪除 `dailyRecommendations`。
- 教師可以讀全校學生及閱讀紀錄。
